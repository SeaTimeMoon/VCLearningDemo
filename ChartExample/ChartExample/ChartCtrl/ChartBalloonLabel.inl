...
void CChartBalloonLabel<PointType>::Draw(CDC* pDC, unsigned uPointIndex)
{
	...
	CPen newPen(PS_SOLID, 2, m_colLine);
	CPen* pOldPen = pDC->SelectObject(&newPen);

	// Draw first the arrow
	pDC->MoveTo(screenPt);
	pDC->LineTo(screenPt.x+10,screenPt.y-20);//balloonlabel 设在右斜角
	...
	CRect temp = labelRect;
	int height = pDC->DrawText(m_strLabelText.c_str(),temp,DT_CENTER | DT_WORDBREAK | DT_CALCRECT | DT_EDITCONTROL); // 获得文本高度
	labelRect.DeflateRect(0,(labelRect.Height() - height) / 2); // 改变rect
	...
	if (m_bRoundedRect)
		...
		pDC->Rectangle(labelRect);
	// Draw the text
	pDC->DrawText(m_strLabelText.c_str(),labelRect, DT_CENTER | DT_EDITCONTROL | DT_WORDBREAK);
