...
//坐标轴的范围
typedef struct {
	double minValue;//坐标轴的最大值
	double maxValue;//坐标轴的最小值
} AxisPos; 
...
typedef struct{
	double minValue;//坐标轴的最大值
	double maxValue;//坐标轴的最小值
	TimeInterval baseInterval; //基准刻度单位
	int Multiplier; //刻度间隔值
	TChartString strFormat; //坐标轴的显示形式
} DateTimeAxisInfo;
...
public:
   ...
   virtual double ScreenToValue(long ScreenVal) const;
	...
	virtual void SetTimeInterval(TimeInterval timeinterval){}
	...
	//返回放大前的状态
	void UndoZoom(double zoomMin,double zoomMax);
	//保存放大前的坐标信息
	...
	//将坐标轴的信息添加进去
	virtual void PushTimeAxisPos(DateTimeAxisInfo timeAxisInfo){}
	virtual void PushStandardAxisPos(AxisPos pos){m_StackStandardAxisPos.push(pos);}
	//将坐标轴的信息删除
	virtual void PopTimeAxisPos(){}
	virtual void PopStandardAxisPos(){m_StackStandardAxisPos.pop();}
	//判断坐标轴保存信息栈是否为空
	virtual bool IsTimeAxisStackEmpty(){return true;}
	virtual bool IsStandardAxisStackEmpty();
	//获得栈顶元素
	virtual void TopTimeAxisStack(DateTimeAxisInfo &timeAxisInfo){}
	virtual void TopStandardAxisStack(AxisPos &axisPos){axisPos=m_StackStandardAxisPos.top();}
	//获得栈的元素个数
	virtual int GetTimeAxisStackSize(){return 0;}
	virtual int GetStandardAxisStackSize(){return m_StackStandardAxisPos.size();}
...
private:
	...
	typedef std::stack<AxisPos> StackAxisPos;
	StackAxisPos m_StackStandardAxisPos; //坐标轴的位置范围
