...
void CChartCtrl::UndoPanZoom()
{
	...
	if (m_pAxes[BottomAxis])
	{
		...
		if (!m_pAxes[BottomAxis]->IsTimeAxisStackEmpty())
		{
			DateTimeAxisInfo timeAxiInfo;
			m_pAxes[BottomAxis]->TopTimeAxisStack(timeAxiInfo);

			m_pAxes[BottomAxis]->SetTickIncrement(false,timeAxiInfo.baseInterval,timeAxiInfo.Multiplier);
			m_pAxes[BottomAxis]->SetTickLabelFormat(false,timeAxiInfo.strFormat);
			m_pAxes[BottomAxis]->UndoZoom(timeAxiInfo.minValue,timeAxiInfo.maxValue);
		}
		else
		{
			m_pAxes[BottomAxis]->SetTickIncrement(false,tiMonth,1);
			m_pAxes[BottomAxis]->SetTickLabelFormat(false,_T("%m-%d"));
	...
	if (m_pAxes[LeftAxis])
	{
		...
		if (!m_pAxes[LeftAxis]->IsStandardAxisStackEmpty())
		{
			AxisPos apos;
			m_pAxes[LeftAxis]->TopStandardAxisStack(apos);
			m_pAxes[LeftAxis]->UndoZoom(apos.minValue,apos.maxValue);
		}
		else
		{
			m_pAxes[LeftAxis]->UndoZoom();
...
void CChartCtrl::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	if (m_bZoomEnabled)
	{
			 ...
			 (m_rectZoomArea.top > m_rectZoomArea.bottom))
		{
			...
			if (!m_pAxes[BottomAxis]->IsTimeAxisStackEmpty())
			{	
				if (m_pAxes[BottomAxis]->GetTimeAxisStackSize()>1) //初始的放大区域信息不删除
				{
					m_pAxes[BottomAxis]->PopTimeAxisPos();
				}
				
			}
			if (!m_pAxes[LeftAxis]->IsStandardAxisStackEmpty())
			{
				if (m_pAxes[LeftAxis]->GetStandardAxisStackSize()>1)
				{
					m_pAxes[LeftAxis]->PopStandardAxisPos();
				}
				  ...
				  (m_rectZoomArea.top!=m_rectZoomArea.bottom))
		{
			...
			if (m_pAxes[BottomAxis])
			{
				...
				{			
					...
					SetTimeAxisZoomMode(MinVal,MaxVal,m_pAxes[BottomAxis]);
			...
			if (m_pAxes[LeftAxis])
			{
				...
				else
				{
					...
					//保存放大的位置信息
					AxisPos axpos;
					axpos.minValue=MinVal;
					axpos.maxValue=MaxVal;

					m_pAxes[LeftAxis]->PushStandardAxisPos(axpos);					
...
void CChartCtrl::SetTimeAxisZoomMode( COleDateTime minVal,COleDateTime maxVal,CChartAxis* pAxis )
{
	COleDateTime MinDate=ValueToDate(minVal);
	COleDateTime MaxDate=ValueToDate(maxVal);
	int modeDays,modeHours,modeMinutes;
	pAxis->GetZoomDateSwitchMode(modeDays,modeHours,modeMinutes);
	COleDateTimeSpan MonthToDay(modeDays,0,0,0);//"月"模式切换为"天"模式临界值
	COleDateTimeSpan DaytoHour(0,modeHours,0,0);//"天"模式切换为"小时"模式临界值
	COleDateTimeSpan HourtoMinute(0,0,modeMinutes,0);//"小时"模式切换为"分钟"模式临界值

	DateTimeAxisInfo timeAxiInfo;
	timeAxiInfo.maxValue=maxVal;
	timeAxiInfo.minValue=minVal;

	if (MaxDate-MinDate>MonthToDay)//时间差超过20天，模式设为"月"
	{

		timeAxiInfo.baseInterval=tiMonth;
		timeAxiInfo.strFormat=_T("%m-%d");
		timeAxiInfo.Multiplier=1;

		pAxis->SetTickIncrement(false,timeAxiInfo.baseInterval,timeAxiInfo.Multiplier);
		pAxis->SetTickLabelFormat(false,timeAxiInfo.strFormat);
	} 
	else if (MaxDate-MinDate>DaytoHour&&MaxDate-MinDate<=MonthToDay)//"天"
	{
		timeAxiInfo.baseInterval=tiDay;
		timeAxiInfo.strFormat=_T("%m-%d");
		timeAxiInfo.Multiplier=1;
	...
	else if(MaxDate-MinDate>HourtoMinute&&MaxDate-MinDate<=DaytoHour)//"时"
	{
		timeAxiInfo.baseInterval=tiHour;
		timeAxiInfo.strFormat=_T("%m-%d %H(h)");
		timeAxiInfo.Multiplier=2;
	...
	{
		timeAxiInfo.baseInterval =tiMinute;
		timeAxiInfo.strFormat=_T("%H:%M");
		timeAxiInfo.Multiplier=10;
	...
	//保存放大的位置信息
	pAxis->PushTimeAxisPos(timeAxiInfo);
