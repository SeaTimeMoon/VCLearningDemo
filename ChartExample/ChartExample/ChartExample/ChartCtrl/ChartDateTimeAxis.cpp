...
CChartDateTimeAxis::CChartDateTimeAxis()
 : CChartAxis(), m_strDTTickFormat(), 
   ...
   m_zoomMModeOverDays(20),
   m_zoomDModeOverHours(24),
   m_zoomHModeOverMinutes(60)
{
...
void CChartDateTimeAxis::SaveAxisInfoBeforeZoom()
{
	DateTimeAxisInfo axisInfo;
	axisInfo.minValue=m_MinValue;
	axisInfo.maxValue=m_MaxValue;
	axisInfo.baseInterval=m_BaseInterval;
	axisInfo.Multiplier=m_iDTTickIntervalMult;
	axisInfo.strFormat=m_strDTTickFormat;

	m_StackTimeAxisPos.push(axisInfo);
	
}

void CChartDateTimeAxis::SetZoomDateSwitchMode( int days,int hours,int minutes )
{
	m_zoomMModeOverDays=days;
	m_zoomDModeOverHours=hours;
	m_zoomHModeOverMinutes=minutes;
}

void CChartDateTimeAxis::GetZoomDateSwitchMode( int &days,int &hours,int &minutes )
{
	days=m_zoomMModeOverDays;
	hours=m_zoomDModeOverHours;
	minutes=m_zoomHModeOverMinutes;

}

bool CChartDateTimeAxis::IsTimeAxisStackEmpty()
{
	bool IsEmpty=m_StackTimeAxisPos.empty();
	return IsEmpty;
