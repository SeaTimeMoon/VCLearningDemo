...
void CListBoxST::MeasureItem(LPMEASUREITEMSTRUCT lpMeasureItemStruct)
{
	...
	GetItemRect(lpMeasureItemStruct->itemID,&csRect);
...
DWORD CListBoxST::OnDrawIcon(int nIndex, CDC* pDC, CRect* prcItem, CRect* prcIcon, int nImage, BOOL bIsDisabled, BOOL bIsSelected)
{
	...
	if (hIcon)
	{
		...
		CPoint	Point(prcIcon->left + CX_BORDER, prcIcon->top +(prcItem->Height()-Size.cy)/2);
...
void CListBoxST::PreSubclassWindow()
{
...
UINT CListBoxST::ItemFromPoint2(CPoint pt,BOOL& bOutside) const
{
...
INT_PTR CListBoxST::OnToolHitTest(CPoint point, TOOLINFO* pTI) const
{
	// TODO: 在此添加专用代码和/或调用基类
 ...
 BOOL CListBoxST::OnToolTipText(UINT nID, NMHDR* pNMHDR, LRESULT* pResult)
 {
	 TOOLTIPTEXTA* pTTTA=(TOOLTIPTEXTA*)pNMHDR;
	 TOOLTIPTEXTW* pTTTW=(TOOLTIPTEXTW*)pNMHDR;
	 char szAText[MAX_PATH]={0};
	 wchar_t szWText[MAX_PATH]={0};

	 CString strTipText;
	 UINT uID = pNMHDR->idFrom;
	 GetText(uID ,strTipText);
	 ::SendMessage(pNMHDR->hwndFrom, TTM_SETMAXTIPWIDTH, 0, 400);

#ifndef _UNICODE
	 if(pNMHDR->code==TTN_NEEDTEXTA)
	 {
		  pTTTA->lpszText=szAText;
		 lstrcpyn(szAText,strTipText,MAX_PATH);
	 }
	 else
	 {
		  pTTTW->lpszText=szWText;
		 _mbstowcsz(szWText,strTipText,MAX_PATH);
	 }
#else
	 if(pNMHDR->code==TTN_NEEDTEXTA)
	 {
		  ...
		  pTTTA->lpszText=szAText;
		 _wcstombsz(szAText,strTipText,MAX_PATH);
	 }
	 else
	 {
		 pTTTW->lpszText=szWText;
		 lstrcpyn(szWText,strTipText,MAX_PATH);
	 }
#endif
